...
namespace Account.API.Grpc
{
    public class AccountGrpcService : AccountGrpc.AccountGrpcBase
    {
        ...
        public override async Task<AccountResponse> PasswordSignIn(AccountRequest request, ServerCallContext context)
        {
            var account = await _accountService.PasswordSignInAsync(request.AccountName, request.Password);
            if (account != null)
            {
                return MapResponse(account);
            }
        ...
        private AccountResponse MapResponse(AccountEntity  account)
        {
            return new AccountResponse()
            {
        ...
        private AccountEntity MapRequest(AccountRequest account)
        {
